...
import { getGramaOptionsForStana, getStanaOptions } from "@/lib/client-utils";
import { LABELS } from "@/lib/labels";

interface AddUpavathiFormProps {
  mandal: Mandal;
  onAdded: () => void;
}
...
export default function AddUpavathiForm({ mandal, onAdded }: AddUpavathiFormProps) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const res = await fetch("/api/upavathis", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ stanaId, gramaId: effectiveGramaId, name }),
      });
      const data = await res.json();
      if (!res.ok) {
        setError(data.error || `Failed to add ${LABELS.upavasathi.toLowerCase()}`);
        return;
      }
      setName("");
      setSuccess(`${LABELS.upavasathi} "${data.name}" added successfully`);
      onAdded();
    } catch {
  ...
  return (
    <form onSubmit={handleSubmit} className="overflow-hidden rounded-2xl border border-orange-100 bg-white shadow-md">
      <div className="border-b border-orange-100 bg-gradient-to-r from-orange-50 to-amber-50 px-6 py-4">
        <h2 className="text-lg font-bold text-stone-900">Add New {LABELS.upavasathi}</h2>
        <p className="text-sm text-stone-500">
          Create a {LABELS.upavasathi.toLowerCase()} under a {LABELS.grama.toLowerCase()}
        </p>
      </div>
      <div className="space-y-4 p-6">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="block text-sm">
            ...
            <select
              ...
              onChange={(e) => setGramaId(e.target.value)}
              className={inputCls}
            >
              {gramaOptions.length === 0 ? (
                <option value="">No {LABELS.grama.toLowerCase()}s in this {LABELS.mandala.toLowerCase()}</option>
              ) : (
                gramaOptions.map((g) => (
        ...
        <label className="block text-sm">
          <span className="mb-1.5 block font-semibold text-stone-700">{LABELS.upavasathi} Name *</span>
          <input
            required
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="e.g. Kundapura Upavasathi"
            className={inputCls}
          />
        </label>
        ...
        <button
          type="submit"
          disabled={loading || gramaOptions.length === 0}
        ...
        >
          {loading ? "Adding..." : `+ Add ${LABELS.upavasathi}`}
        </button>
      </div>
    </form>
  );
